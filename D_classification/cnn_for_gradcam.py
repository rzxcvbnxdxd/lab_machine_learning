...
shutil.copyfile(r"C:\Users\ylab\Desktop\研究プログラム\ingot-main\D_classification\settings.py", rf"{output_dir}\settings.py")
...
class dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        # GPU 上に直接置かない。学習ループ側で .to(device) する。
...
class Network(nn.Module):
    def __init__(self, num_layers=6, num_filters=64, out_features=256, dropout_rate=0.5):
        ...
        for i in range(num_layers):
            if i == 0:
                conv_modules.append(nn.Conv1d(feature_num, num_filters, kernel_size=3, stride=1))
            else:
                conv_modules.append(nn.Conv1d(num_filters, num_filters, kernel_size=3, stride=1))
            conv_modules.append(nn.BatchNorm1d(num_filters))
            conv_modules.append(nn.ReLU())
            conv_modules.append(nn.MaxPool1d(kernel_size=2, stride=2))
        ...
        # 畳み込み特徴抽出部と分類器を分離しておく（Grad-CAM 用に最後の conv に hook を付けやすくする）
        self.features = nn.Sequential(*conv_modules)

        self.classifier = nn.Sequential(
            nn.Dropout(dropout_rate),
            nn.Flatten(),
            nn.Linear(num_filters * features, out_features),
            nn.ReLU(),
            nn.Dropout(dropout_rate),
...
def init_weights(m):  # Heの初期化
    if type(m) == nn.Linear or type(m) == nn.Conv1d:
        ...
        # m.to(device) はここでは行わない（モデル全体を一箇所で .to(device) する）
...
def train(net, optimizer, patience=10):
    ...
    while True:
        ...
        for X, y in dataloader_train:
            ...
            # CrossEntropyLoss はクラスインデックス（LongTensor）を期待するため one-hot を渡さない
            loss = loss_function(y_pred_hot, y)  # 誤差(クロスエントロピー誤差関数)の計算
