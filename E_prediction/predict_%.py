...
with open(rf'{model_dir}\info.json', 'r', encoding='utf-8') as f:
...
data_path = filedialog.askopenfilename(title="select measurement data", filetypes=[('file', '*.txt *.csv')])
if not data_path:
    sys.exit()
...
def evaluation(net, data):
    ...
    for y_pred, y_pred_hot in zip(y_pred_list, y_pred_hot_list):
        if y_pred_hot[y_pred] > 0.5:
            count[y_pred] += 1
        else:  # その他
            count[-1] += 1

    total_count = sum(count)
    percent_count = [(c / total_count) * 100 for c in count]  # パーセンテージに変換
    ...
    # 棒グラフの塗りつぶしを水色に戻す
    bars = ax.bar(indices, percent_count, color='skyblue', edgecolor='black', linewidth=0.7, width=bar_width, zorder=3)

    for x, pc in zip(indices, percent_count):
        plt.text(x, pc, f'{pc:.1f}%', ha='center', va='bottom', fontsize=10, zorder=4)
    ...
    plt.ylabel('Percentage (%)', fontsize=12, fontweight='bold')  # Y軸ラベルを太字に
    plt.yticks(fontsize=10)
    ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f'{x:.0f}%'))  # Y軸をパーセント表示に

    plt.savefig(rf'{data_dir}\classification_result_{measurement_freqs}.svg', format='svg')
    plt.show()
    plt.close()
...
def main():
    ...
    # 保存された重みをロード
    net.load_state_dict(torch.load(rf'{model_dir}/model.pth', torch.device('cpu')))
